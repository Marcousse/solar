...
/**
 * UI window for creating new object in a specific position, with a specific
 * velocity.
 */
public class NewObjPosWindow extends JDialog {
	...
	public NewObjPosWindow(Frame mainWindow, boolean isModal) {
		...
		panel5.add(coordLabel);
		panel5.add(xField);
		panel5.add(yField);

		vxField = new JTextField(3);
		JLabel vcoordLabel = new JLabel("Velocity(x,y):");
		vyField = new JTextField(3);
		panel6.add(vcoordLabel);
		panel6.add(vxField);
		panel6.add(vyField);
		...
		createButton.addActionListener((ActionEvent e) -> {
			...
			Vector2 position = new Vector2((double) Integer.parseInt(xField.getText()),
					(double) Integer.parseInt(yField.getText()));
			Color color = (Color) colorPicker.getSelectedItem();
			Vector2 velocity = new Vector2((double) Integer.parseInt(vxField.getText()),
					(double) Integer.parseInt(vyField.getText()));
			mainWindow.getController().newObject(name, gravMass, inerMass, radius, position, velocity, color);
			dispose();
		});
